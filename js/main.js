...
// download btn start
 function downloadPDF() {
      ...
      link.href = 'PDF%202.pdf'; // must be on same domain
      link.download = 'PDF 2.pdf';
...
// Get elements by class and id
    const discountButton = document.querySelector('.btn-discount');
     const discountButton1 = document.querySelector('.btn-discount1');
    const paymentPopup = document.getElementById('paymentPopup');
     const paymentPopup1 = document.getElementById('paymentPopup1');
    const closePopup = document.getElementById('closePopup');
      const closePopup1 = document.getElementById('closePopup1');

    // Open the popup when the discount button is clicked
    discountButton.onclick = () => paymentPopup.style.display = 'block';
    discountButton1.onclick = () => paymentPopup1.style.display = 'block';

    // Close the popup when the close button is clicked
    closePopup.onclick = () => paymentPopup.style.display = 'none';
    closePopup1.onclick = () => paymentPopup1.style.display = 'none';

    // Close the popup if the user clicks outside the popup container
    window.onclick = (event) => {
     ...
     window.onclick = (event) => {
      if (event.target === paymentPopup1) {
        paymentPopup1.style.display = 'none';
      }
    };

    // Handle pre-order form submission
    document.getElementById('preOrderForm').onsubmit = (event) => {
      event.preventDefault();
      alert("Thank you for your pre-order! You will receive a confirmation email soon.");
      paymentPopup.style.display = 'none';
    };
     document.getElementById('preOrderForm1').onsubmit = (event) => {
      event.preventDefault();
      alert("Thank you for your pre-order! You will receive a confirmation email soon.");
      paymentPopup1.style.display = 'none';
    };
...
// deep dive pop-up start
// Open modal
    function openModal() {
      document.getElementById("paymentModal").style.display = "block";
      document.getElementById("paymentModal1").style.display = "block";
    }

    // Function to close the modal
    function closeModal() {
      document.getElementById("paymentModal").style.display = "none";
      document.getElementById("paymentModal1").style.display = "none";
    }
    ...
    window.onclick = function(event) {
      ...
      window.onclick = function(event) {
      const modal = document.getElementById("paymentModal1");
      if (event.target === modal) {
        closeModal();
      }
    }
        // deep dive pop-up end
...
jQuery(document).ready(function($) {
  $('.btn-view').on('click', function(){
      ...
      let _modal = $(this).closest('div').find('.modal');
